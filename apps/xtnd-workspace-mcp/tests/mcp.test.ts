...
test('MCP Server: lists graph tools (search, get, query, assert, write_back)', () => {
  ...
  assert.ok(tools.some((t: any) => t.name === 'xworkspace_search_nodes'));
  assert.ok(tools.some((t: any) => t.name === 'xworkspace_query_graph'));
  assert.ok(tools.some((t: any) => t.name === 'xworkspace_write_back'));
});
